...
class TestLinkAPI
  ...
  def getLatestBuildIDForTestPlan(testplanid)
    ...
    resp.class == Array ? nil : resp['id'].to_i
  end
  ...
  def addTestCaseKeywords(keywords)
    args={"devKey"=>@devkey, "keywords"=>keywords}
    res=@tl.call("tl.addTestCaseKeywords", args)
    res
  end

  def updateTestCaseCustomFieldDesignValue(testcaseexternalid, version, test_project, customfields)
    args={"devKey"=>@devkey, "testcaseexternalid"=>testcaseexternalid, "version"=> version,
          "testprojectid"=> test_project, "customfields"=> customfields}
    res=@tl.call("tl.updateTestCaseCustomFieldDesignValue", args)
    puts res
  end
  ...
  def getChildTestSuiteIDByName(child_name, parent_id)
    ...
    # I am testing the value of the first pair to see if it's a hash.
      ...
      if resp.first[1].is_a? Hash # many children
        resp.each do |_id, hash|
  ...
  def get_test_case_ids_from_path(test_project, test_folder_path)
    ...
    test_cases.each_with_object({}) { |test_case, hash| hash[test_case['name']] = test_case['id'].to_i; }
  end
